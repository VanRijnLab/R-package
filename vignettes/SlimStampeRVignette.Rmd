...
There are NA values in your data set. The plots for the sessionId’s that contain NA values are still printed, but the rows that contain a NA value are removed and not printed as a data point. The first line that is printed after the function should clarify which column contains NA values. Having NA values in your data can also lead to the axis of the plots being printed in unexpected ways. 
